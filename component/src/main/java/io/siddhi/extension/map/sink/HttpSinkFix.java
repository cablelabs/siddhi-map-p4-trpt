 ...
 */
package io.siddhi.extension.map.sink;
 ...
 */
@Extension(name = "http-fix", namespace = "sink",
        description = "" +
...
public class HttpSinkFix extends Sink {
    private static final Logger log = Logger.getLogger(HttpSinkFix.class);
//    String mapType;
...
//        } else {
            clientConnector = createClientConnector(payload);
//        }
...
//        }
    ...
    protected int sendRequest(Object payload, DynamicOptions dynamicOptions, List<Header> headersList,
        ...
        String contentType = HttpSinkUtil.getContentType(getMapper().getType(), headersList);
        ...
        publisherURL = publisherURLOption.getValue() + '?' + messageBody.substring(1);
        cMessage.setRequestUrl(publisherURL);
...
//        }
    ...
    private void requestForNewAccessToken(Object payload, DynamicOptions dynamicOptions, List<Header> headersList,
                                          String encodedAuth, ClientConnector clientConnector)
            throws ConnectionUnavailableException {
        final boolean checkRefreshToken = accessTokenCache.checkRefreshAvailableKey(encodedAuth);
...
//        }
    ...
    HttpCarbonMessage generateCarbonMessage(List<Header> headers, String contentType,
        ...
        if (contentType.contains(getMapper().getType())) {
    ...
    String getMessageBody(Object payload) {
        if (HttpConstants.MAP_KEYVALUE.equals(getMapper().getType())) {
    ...
    public ClientConnector createClientConnector(final Object payload) {
        String messageBody = getMessageBody(payload);
        publisherURL = publisherURLOption.getValue() + '?' + messageBody.substring(1);
    ...
    private class HTTPResponseListener implements HttpConnectorListener {
        ...
        HttpSinkFix httpSink;
        private final String publisherURL;

        HTTPResponseListener(Object payload, DynamicOptions dynamicOptions, HttpSinkFix httpSink, String publisherURL) {
